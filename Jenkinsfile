  ...
  stages {
    ...
    stage('Fetch the DDL from TEST_DEV DB') {
	  ...
	  stage('Review the Code') {
			steps {
				emailext attachmentsPattern: 'BIDW.DBA.$Objectname', body: "Review the below code" , subject: "Build Number: ${env.BUILD_NUMBER} Job Name: ${env.JOB_NAME}", to: 'yatin.sawant@officedepot.com'
				  }
			}
